...
export function useReporting() {
    ...
    useEffect(() => {
        async function loadOffline() {
            ...
            try {
                ...
                Logger.error('[Reporting] Échec chargement file d\'attente hors-ligne', err);
            }
        }
        loadOffline();
    }, []);
    ...
    const flush = useCallback(async () => {
        ...
        try {
            Logger.info(`[Reporting] Envoi de ${items.length} éléments`, items);
            await sendStats(items);
            if (ConfigManager.isInitialized()) {
                await persistentCache.delete('data', OFFLINE_QUEUE_KEY());
            }
        } catch (error) {
            Logger.error('[Reporting] Échec envoi, remise en file d\'attente', error);
