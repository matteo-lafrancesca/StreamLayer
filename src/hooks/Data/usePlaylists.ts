...
/**
 * Récupère les playlists d'un projet avec pagination et stratégie SWR.
 */
export function usePlaylists(
    ...
    useEffect(() => {
        ...
        const checkCache = async () => {
            ...
            if (cached) {
                Logger.info('[usePlaylists] Succès cache :', cached.length);
                setCachedPlaylists(cached);
            }

            // Revalidation arrière-plan (SWR)
    ...
    // Sauvegarde en cache une fois la pagination terminée
    useEffect(() => {
        if (
            stableCacheKey &&
            paginationKey &&
    ...
    useEffect(() => {
        if (autoRefresh || autoRefreshOption) setRefreshKey((prev: number) => prev + 1);
    }, [autoRefresh, autoRefreshOption]);

    useEffect(() => {
        if (refreshTrigger && !previousRefreshTrigger.current) refreshPlaylists();
