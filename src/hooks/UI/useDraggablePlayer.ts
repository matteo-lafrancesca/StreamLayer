...
// Gère le déplacement (drag & drop) du lecteur mini sur Desktop
// Gère également l'ancrage (docking) automatique sur les bords et l'inertie
...
export function useDraggablePlayer({
  ...
  useEffect(() => {
    if (!isCompact && customPosition) {
      ...
      const targetY =
        window.innerHeight - PLAYER_SIZES.DESKTOP.EXPANDED_HEIGHT - 20;
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const playerHeight =
      initialRect.height || PLAYER_SIZES.DESKTOP.COLLAPSED_HEIGHT;

    const rawDropY = initialRect.top + delta.y;
    const rawDropX = initialRect.left + delta.x;

    const inertiaX =
      rawDropX + velocityRef.current.vx * LERP_CONFIG.inertiaMultiplier * 10;
    const targetX = Math.max(
      20,
      Math.min(inertiaX, windowWidth - playerWidth - 20)
    );
